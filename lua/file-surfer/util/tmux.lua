...
function M.switch_tmux_session(session_name)
    local current_session = run_command("tmux display-message -p '#S'", true)

    local function switch_session(session)
...
function M.switch_tmux_window()
    local current_window_id =
        run_command("tmux display-message -p '#{session_name}:#{window_id}'", true)

    local window_list =
        run_command("tmux list-windows -a -F '#{session_name} #{window_id} #{window_name}'")
        ...
        windows = vim.tbl_filter(function(item)
            ...
            if session and window_id then
                ...
                if
                    item ~= ""
                    and (
                        (current_window_id and full_id ~= current_window_id)
                        or not current_window_id
                    )
                then
...
function M.start_session_in_cwd(name, cwd)
    local cmd_create = string.format("tmux new-session -d -s %s -c %s", name, cwd)
    local cmd_switch = string.format("tmux switch-client -t %s", name)
...
function M.create_tmux_integration_bindings()
    vim.keymap.set("n", "<leader>.c", M.start_custom_tmux_session, { desc = "[C]reate Session" })
    vim.keymap.set("n", "<leader>.s", M.switch_tmux_session, { desc = "Switch [S]ession" })
    vim.keymap.set("n", "<leader>.w", M.switch_tmux_window, { desc = "Switch [W]indows" })
    vim.keymap.set("n", "<leader>.t", M.switch_nvim_instance, { desc = "Switch Neovim [I]nstance" })
end
